          ...
          <table class="orders-table">
            ...
            <tbody>
              <tr v-for="order in submittedOrders" :key="order.id">
                ...
                <td class="col-date">{{ formatDate(order.order_date) }}</td>
                <td class="col-date">{{ formatDate(order.expected_delivery) }}</td>
                <td class="col-date">{{ leadTimeDays(order) }} {{ t('orders.days') }}</td>
                <td class="col-value"><strong>{{ currencySymbol }}{{ order.total_value.toLocaleString() }}</strong></td>
              </tr>
            </tbody>
          </table>
  ...
  setup() {
    ...
    const loadOrders = async () => {
      try {
        ...
        // Restocking orders (status "Submitted") have their own section below, keep them out of this table
    ...
    const leadTimeDays = (order) =>
      Math.round((new Date(order.expected_delivery) - new Date(order.order_date)) / (1000 * 60 * 60 * 24))
